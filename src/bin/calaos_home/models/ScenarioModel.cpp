...
void ScenarioModel::scenario_list_cb(bool success, vector<string> result, void *data)
{
        ...
        for (uint i = 2;i < result.size();i++)
        {
                ...
                {
                        Utils::logger("scenario") << Priority::ERROR << "ScenarioModel: Unknown Input \'" << result[i] << "\' !" << log4cpp::eol;
                        continue;
                }
...
void Scenario::scenario_get_cb(bool success, vector<string> result, void *data)
{
        ...
        for (uint b = 3;b < result.size();b++)
        {
                ...
                else
                {
                        ...
                        if (it == CalaosModel::Instance().getHome()->getCacheOutputs().end())
                        {
                                Utils::logger("scenario") << Priority::ERROR << "ScenarioModel::scenario_get Unknown action id \'" << tmp[0] << "\' with action \'" << tmp[1] << "\' !" << log4cpp::eol;
                                continue;
                        }
                        IOBase *io = (*it).second;
...
void ScenarioModel::notifyScenarioAddDelayed(string notif)
{
        ...
        if (it == CalaosModel::Instance().getHome()->getCacheInputs().end())
        {
                Utils::logger("scenario") << Priority::ERROR << "ScenarioModel: Unknown Input \'" << split_id[1] << "\' !" << log4cpp::eol;
                return;
        }
...
void ScenarioModel::notifyScenarioDel(Scenario *sc)
{
        if (!sc)
        {
                Utils::logger("scenario") << Priority::ERROR << "ScenarioModel::notifyScenarioDel sc is NULL!" << log4cpp::eol;
                return;
        }
        ...
        if (it == scenarios.end())
        {
                Utils::logger("scenario") << Priority::ERROR << "ScenarioModel: Unknown scenario \'" << sc->ioScenario->params["id"] << "\' !" << log4cpp::eol;
                return;
        }
